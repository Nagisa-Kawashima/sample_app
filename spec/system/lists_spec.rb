...
describe '投稿のテスト' do
  ...
  describe 'トップ画面(top_path)のテスト' do
    ...
    context '表示の確認' do
      ...
      it 'top/pathが"/top"であるか' do
        except(current_path).to eq('/top')
      end
    end
  end
end
...
describe "投稿画面(new_list_path)のテスト" do
  ...
  context '表示の確認' do
    it 'new_list_pathが"/lists/new"であるか' do
      except(current_path).to eq('/lists/new')
    end
  ...
  context '投稿処理のテスト' do
    it '投稿後のリダイレクト先は正しいか' do
      ...
      click_button '投稿'
      except(page).to have_current_path list_path(List.last)
    end
  end
end
...
describe "詳細画面のテスト" do
  ...
  context 'list削除のテスト' do
    ...
    context '更新処理に関するテスト' do
      it '更新処理後のリダイレクト先は正しいか' do
        fill_in 'list[title]', with: Faker::Lorem.characters(number:5)
        fill_in 'list[body]', with: Faker::Lorem.characters(number:20)
        click_button '保存'
        except(page).to have_current_path list_path(list)
      end
    end
  end
end
